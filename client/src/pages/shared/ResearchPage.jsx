...
import { researchAPI, getFileUrl, getResearchProposalUrl, getResearchFinalReportUrl } from '../../services/api';
import {
    ...
    return (
        <div className="card hover:shadow-md transition-shadow duration-200">
            ...
            <div className="flex gap-2 pt-3 border-t border-slate-100">
                ...
                {(entry.files?.length > 0 || entry.proposalFile?.filePath || entry.finalReportFile?.filePath) && (
                    <button
                        onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            ...
                            if (entry.proposalFile?.filePath) {
                                filesToDownload.push({
                                    url: getResearchProposalUrl(entry._id),
                                    name: entry.proposalFile.originalName
                                });
                            }
                            if (entry.finalReportFile?.filePath) {
                                filesToDownload.push({
                                    url: getResearchFinalReportUrl(entry._id),
                                    name: entry.finalReportFile.originalName
                                });
                            }
                            if (entry.files && entry.files.length > 0) {
                                entry.files.forEach(f => {
                                    filesToDownload.push({
                                        url: getFileUrl(entry._id, f._id),
                                        name: f.originalName
                                    });
                                });
                            }
                        ...
                        title="Download all research files"
                    >
